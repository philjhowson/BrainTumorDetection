...
def evaluate_densenet(version = 1):
    ...
    checkpoint = torch.load(f'models/densenet_tumor_checkpoint_v{version}.pth', weights_only = True)
    model.load_state_dict(checkpoint)

    with open(f'metrics/densenet_tumor_performance_v{version}.pkl', 'rb') as f:
    ...
    plt.savefig(f'images/densenet_history_v{version}.png', bbox_inches = 'tight');

    print('Training and validation metrics plot saved.')
    ...
    with open(f"metrics/densenet_classification_report_v{version}.pkl", "wb") as f:
        pickle.dump(report_dict, f)

    scores = {'ROC-AUC' : test_roc_score,
              'F1 Score' : test_f1_score}

    with open(f"metrics/densenet_scores_v{version}.pkl", "wb") as f:
        pickle.dump(scores, f)

    print('Test scores and classification report saved.')
